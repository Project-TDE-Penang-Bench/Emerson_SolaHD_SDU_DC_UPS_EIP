...
def is_module_detected():
    ...
    return (pyautogui.locateOnScreen('./Picture/module_detected_post_bfu_eip.png', confidence=0.9) != None) or (pyautogui.locateOnScreen('./Picture/module_detected_pre_bfu_eip.png', confidence=0.9) != None) or (pyautogui.locateOnScreen('./Picture/module_detected_post_bfu_network_changed_eip.png', confidence=0.9) != None)
...
try:
    ...
    # Check for "Update Module" button to determine if in "Change Network" page
    if (pyautogui.locateOnScreen('./Picture/is_update_module.png', confidence=0.9) != None):
        print("run-1")
        # Navigate to "Change Network" page
        change_network_type()
        
        # Update network to Modbus if not already
        if not (is_network_MBUS()):
            ...
            if not (is_firmware_updated()):
                ...
                print("MOSBUC FW IS LATEST. NO UPDATE REQUIRED.")
                print("PASS")
                app.window(title='Firmware Manager II').close()
