...
var LEDrefreshInterval = 1000;
var count = 0;
var prv_Val;
var inte_count = 0;
var intReqCoutn = 1;
//debugger;
objParaRefresh = setInterval(()=>{
    ...
    if(mp.get(102) != ""){
        ...
        /* if(mp.get(109) != ""){
           // debugger;       
            //var strtemp109 = `${mp.get(109)} °C , ${(((mp.get(109))*(9/5))+32)} °F` 
            //$("#109").html((`(${mp.get(109)} °C), (${(((mp.get(109))*(9/5))+32).toFixed(2)} °F)`));
            //dC2dF(mp.get(109));
            //console.log(`string => ${dC2dF(mp.get(109))}`) ; 
            $("#109").html(dC2dF(mp.get(109)));
        } */

        $("#400").html(mp.get(400));

        $("#402").html(mp.get(402));
        /* if(mp.get(402) == 'FF' || mp.get(402) == '01')
            $("#402").prop('checked', true);
        else
...
//fetchLEDStatus();
objFetchLEDStatus = setInterval(()=>{fetchLEDStatus(); }, LEDrefreshInterval);

function fnStopFetchLEDStatus(){
    if (objFetchLEDStatus != null) {
        clearInterval(objFetchLEDStatus);
    }
}

function fnStopParaRefresh(){
    if (objParaRefresh != null)
        clearInterval(objParaRefresh);
}
