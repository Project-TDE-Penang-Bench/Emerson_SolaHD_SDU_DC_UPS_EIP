...
$.getJSON(
    ...
    .then((result) => {
        inputJSON = result;
        //debugger;
        objDataOperationRefresh_1 = setInterval(()=>{fnFetchParaData();}, refreshInterval);
        //fnStartDataOperationRefresh();
    })

    async function fnFetchParaData() {
    //console.log('in method => fnFetchParaData() inside dataoperation.js page...');
...
function fnStartDataOperationRefresh(){
    //console.log(`inside fnStartDataOperationRefresh()...`);
    //objDataOperationRefresh_1 = setInterval(await fnFetchParaData(), refreshInterval);
    objDataOperationRefresh_1 = setInterval(()=>{fnFetchParaData();}, refreshInterval);
}

function fnStopDataOperationRefresh() {
    //console.log(`inside fnStopDataOperationRefresh()...`);
    if (objDataOperationRefresh_1 != null) {
        clearInterval(objDataOperationRefresh_1);
        //objDataOperationRefresh = null;
        objDataOperationRefresh_1 = null;
    }
}
...
function paramConvrt_v1(instID, paramName, paramHexVal) {
    ...
    switch (instID) {
        ...
        case 212:// SDN1 LED status
            ...
            else if (paramHexVal == "05" || paramHexVal == "5")
                retValue = '<span>Normal Operation</span>'; //  "Heavy Load";
            else if (paramHexVal == "06" || paramHexVal == "6")
        ...
        case 6:
            retValue = parseInt(_paramHexVal, 16);
            retValue = retValue.toLocaleString('en-US',{minimumIntegerDigits: 2, useGrouping: false});
            break;
        case 7:
            retValue = parseInt(_paramHexVal, 16);
            break;
